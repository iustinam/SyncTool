...
###### GLOBAL CONFIG TAB
my ($frTabConf_head,$lblTabConf_errors,$frTabConf_Up,$frTabConf_Left,$frTabConf_Right,$frTabConf_Warns,$frTabConf_Bottom);
my (@label_order,$confEntries,$confWarns,$confVariables,@order);
###### JOBS TAB
...
sub validatePaths{
    for (keys %conf_valid){ #reset
        undef $conf_valid{$_};
    }
    
    my $running_folder=$paths->{running_folder};
    if((not $running_folder)||(not -d $running_folder)){
       print "gui: loadPaths: no/invalid running_folder\n";
       $conf_valid{running_folder}="* Invalid (Mandatory).";
    }else{
...
sub main(){
    ...
    &loadPaths($xml_conf);
    #todo in running folder missing.. die
    if($conf_valid{running_folder}||$conf_valid{jobs_conf_filename}||$conf_valid{sync_path}){
        die "Configuration file ($xml_conf) must have valid values for 'running_folder','jobs_conf_filename' and 'sync_path' in order to start\n";
    }
    &redirectSTDOUT();
...
sub paintServiceTab{
    ...
    $btnNotifyConf=$frTabService_Bottom->Button(-width=>40,-text=>"Notify Jobs Reconfiguration",-command=>sub{
        ...
        if((scalar @portfiles)>1){
            ...
            my @sorted=sort{-M File::Spec->catfile($PORFILE_FOLDER,$a) <=> -M File::Spec->catfile($PORFILE_FOLDER,$b)} @portfiles;
            $theFile=$sorted[0];
        }elsif((scalar @portfiles)<1){
            print "warning: no service running file was found\n";   
        }elsif((scalar @portfiles)==1){
            $theFile=$portfiles[0];
        }
        if($theFile){ #if one was found
            $theFile=~m/service_running\.([0-9]*)$/;
            print $1."\n";
            if($1){
...
sub paintTabGlobalConfig{
    ...
    $lblTabConf_errors = $frTabConf_head->Label(-text =>" ", -justify => 'left')->pack(-side => "bottom", -anchor => "nw");
    ...
    foreach(@order) {
        ...
        $confWarns->{$_}=$frTabConf_Warns->Label(-text =>$conf_valid{$_}, -justify => 'left', -foreground=>'red' )->pack(-side => "top", -anchor => "nw");
    }
    ...
    $frTabConf_Bottom->Button(-text=>"Save",-command=>sub{
            #clean warnings
            for(keys %$confWarns){
                $confWarns->{$_}->configure(-text =>"");    
            }
            
            my $nok_jobconf=0;
            if((not ($confVariables->{jobs_conf_filename} eq $paths->{jobs_conf_filename}))
                or (not ($confVariables->{running_folder} eq $paths->{running_folder}))){
            ...
            &validatePaths();   
            foreach(@order) { # validation messages 
                $confWarns->{$_}->configure(-text =>$conf_valid{$_}) if $conf_valid{$_};
            }
            if($conf_valid{running_folder}||$conf_valid{jobs_conf_filename}||$conf_valid{sync_path}){
                $lblTabConf_errors->configure(-text=>"Configuration was not saved.", -foreground=>'red');
                #could not disable.. err .. solution: NOT SAVING
            }else{
                &saveConf();
                #print "saving conf\n";
                $lblTabConf_errors->configure(-text=>"Configuration was saved successfully.",-foreground=>'DarkGreen');
                if($nok_jobconf){ # we have a new(valid) jobs conf file, recreate our jobs hash
...
#from paths to entries
sub resetConfEntries{
    foreach(keys %$paths){
        if($confEntries->{$_}){
            $confEntries->{$_}->delete(0,'end'); 
            $confEntries->{$_}->insert(0,$paths->{$_}) if (not ref $paths->{$_} eq "HASH"); #else it would appear HASH(0x......) instead of empty value
        }
    }
}
#from entries to paths
...
sub delete_job{
    my $sid = shift || undef;
    return if not $sid;
    my $response=$mw->messageBox(-title=>"",-message=>"Delete job $conf->{job}{$sid}{title} ?",-type=>'yesno',-icon=>'question');
...
#    my $cli=$s->accept();
    ...
    while($cli->recv(my $recv_data,1024)){
        ...
        print "gui: handle_one_proc: recv @cmd \n";
        {
            ...
            $info{$cmd[0]}{$cmd[1]}{TIME}=$cmd[8];#int($cmd[8]/3600).":".int(($cmd[8]%3600)/60).":".$cmd[8]%60;
...
#        }
    ...
    my $document = do {
        local $/ = undef;
        open my $fh, "<",$jobsConf or die("could not open : $!");#($mw->destroy()&& die("could not open : $!"));
        <$fh>;
    };
...
sub restoreConf{
    ...
    # do not load configurations that have invalid mandatory values
    my $first=1;
    do{
        if(not $first){
            $mw->messageBox(-title=>"Invalid configuration",-message=>"Invalid configuration. Please choose another file. ",-type=>'ok',-icon=>'error');
        }
        ...
        &loadPaths($fname);
        $first=0;
    }while($conf_valid{running_folder}||$conf_valid{jobs_conf_filename}||$conf_valid{sync_path});
