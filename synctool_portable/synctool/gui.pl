...
my $t_frame = $mw->Frame()->pack(-side => 'top');

my $b_frame = $mw->Frame()->pack(-side => 'bottom',-padx => 10,-pady => 10);
...
my $btn_close = $b_frame->Button(-text => "Close",-command => sub{bbye();})->pack();
##############################################################################
...
sub readXmlJobs{
    ...
    my $bkp=$jobs_confFile.POSIX::strftime("_%H.%M.%S_%d.%m.%y",localtime).".bkp";
    if ( system( 'copy', "\"$jobs_confFile\"", "\"$bkp\"" ) ) {
        print "gui: readXmlJobs: err backing up $jobs_confFile \n ";
    }
