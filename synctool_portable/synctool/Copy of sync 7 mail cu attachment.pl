...
sub sync_starter{
    ...
    $SIG{'KILL'}=sub{ print   "----".threads->self()->tid()." sync : starter thr: dying..\n";threads->exit() };
    ...
    #validate smtp server reachable
    ...
    if($content){ #&&$doSend){
        foreach(keys %{$opt{mails}}){
            if($opt{mails}{$_}){
                ...
                if(!$msg){
                    print $logh "Sending mail to $_: Error creating multipart container : $!\n"; 
                    next;
                }
