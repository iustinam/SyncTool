...
#    }
    ...
    my @l=grep /($excl_re)/,@ls;
    $log{F}{IGNORE}+=scalar @l if scalar @l;
    @ret=grep !/($excl_re)/,@ls;
...
#    }
    ...
    my @l=grep $del{File::Spec->catfile( $src, $_ )} || $del{File::Spec->catfile( $dst, $_ )}, @all;
    $log{F}{IGNORE}+=scalar @l if scalar @l;
...
sub write_stats_to_handle{
    ...
    print $logh "Summary: (total ignored items):".$log->{F}{IGNORE}."\n";
