...
$frTabConf_Right->Entry(-textvariable =>\$paths->{running_folder}, -justify => 'left')->pack(-side => "top", -anchor => "nw");
$frTabConf_Right->Entry(-textvariable =>\$paths->{jobs_conf_filename}, -justify => 'left')->pack(-side => "top", -anchor => "nw");
$frTabConf_Right->Entry(-textvariable =>\$paths->{sync_path}, -justify => 'left')->pack(-side => "top", -anchor => "nw");
$frTabConf_Right->Entry(-textvariable =>\$paths->{portfiles}, -justify => 'left')->pack(-side => "top", -anchor => "nw");
$frTabConf_Right->Entry(-textvariable =>\$paths->{logs_folder}, -justify => 'left')->pack(-side => "top", -anchor => "nw");
$frTabConf_Right->Entry(-textvariable =>\$paths->{bkp_folder}, -justify => 'left')->pack(-side => "top", -anchor => "nw");
$frTabConf_Right->Entry(-textvariable =>\$paths->{touch_portfile_time}, -justify => 'left')->pack(-side => "top", -anchor => "nw");
$frTabConf_Right->Entry(-textvariable =>\$paths->{email_server}, -justify => 'left')->pack(-side => "top", -anchor => "nw");
...
$MW_listBox = $l_frame->Scrolled("Listbox", -selectmode => "single", -width => "50", -height => "10",  #?in ce il masoara
...
my $btn_reload = $b_frame->Button(-text => "Restore Config",-command => sub{reloadJobsConf();})->pack(-side => 'left',-anchor=> 'nw', -padx => 3);
##############################################################################
...
sub edit_job{
    ...
    $l0_frame->LabEntry(-label =>"Job Name", -labelPack=>[-side => "left", -anchor => "w"], 
                        -width=>30, -textvariable=>\$tmpJob->{title} ) # or.. $edit_sid?\$edit_sid:undef
            ->pack(-side => "top", -anchor => "nw");
#    $l0_frame->Label(-text=>"Job Name:   $tmpJob->{title}")
#            ->pack(-side => "top", -anchor => "nw");
    $l0_frame->Label(-text => "________________________________________")
        ->pack(-side => 'top',-anchor=> 'nw', -padx => 1);
        
    $l1_frame->LabEntry(-label =>"Master share", -labelPack=>[-side => "left", -anchor => "w"], 
                        -width=>25, -textvariable=>\$tmpJob->{master} ) 
    ...
    my $l3_frame   = $l_frame->Frame->grid(-row => 3,-column => 0,-sticky => 'nw')->pack(-side => 'top',-fill=>'x',-padx => 1, -pady => 1.5);
    $new_listBox_cli = $l3_frame->Scrolled("Listbox", -selectmode => "single", -width => "45", -height => "15",  #?in ce il masoara
		  -scrollbars => 'osoe');
    ...
    $new_listBox_cli->pack(-side => 'top',-anchor=> 'nw');  
    #################
    
    ################# RIGHT FRAME
    my $r0_frame   = $r_frame->Frame->grid(-row => 0,-column => 0,-sticky => 'nw')->pack(-side => 'top',-fill=>'x',-padx => 1, -pady => 1.5);
    my $r1_frame   = $r_frame->Frame->grid(-row => 1,-column => 0,-sticky => 'nw')->pack(-side => 'top',-fill=>'x',-padx => 1, -pady => 1.5);
    my $r2_frame   = $r_frame->Frame->grid(-row => 2,-column => 0,-sticky => 'nw')->pack(-side => 'top',-fill=>'x',-padx => 1, -pady => 1.5);
    my $r3_frame   = $r_frame->Frame->grid(-row => 3,-column => 0,-sticky => 'nw')->pack(-side => 'top',-fill=>'x',-padx => 1, -pady => 1.5);
    my $r4_frame   = $r_frame->Frame->grid(-row => 4,-column => 0,-sticky => 'nw')->pack(-side => 'top',-fill=>'x',-padx => 1, -pady => 1.5);
    my $r5_frame   = $r_frame->Frame->grid(-row => 5,-column => 0,-sticky => 'nw')->pack(-side => 'top',-fill=>'x',-padx => 1, -pady => 1.5);
    my $r6_frame   = $r_frame->Frame->grid(-row => 6,-column => 0,-sticky => 'nw')->pack(-side => 'top',-fill=>'x',-padx => 1, -pady => 1.5);
    ...
    $r1_frame->Label(-text => "________________________________________")
        ->pack(-side => 'top',-anchor=> 'nw', -padx => 1);
    $r2_frame->LabEntry(-label =>"Exclude Paths", -labelPack=>[-side => "left", -anchor => "w"], 
    ...
    ########## exclude regex area
    $r4_frame->Label(-text => "________________________________________")  
        ->pack(-side => 'top',-anchor=> 'nw', -padx => 1);
    
    $r5_frame->LabEntry(-label =>"Exclude Words", -labelPack=>[-side => "left", -anchor => "w"], 
                        -width=>25,-textvariable => \$new_exclude_re,-background   => 'gray',)
        ->pack(-side => 'left',-anchor=> 'nw', -padx => 1);
    
    $r5_frame->Button(-text=>"+", -command => sub{
            if($new_exclude_re){
                my $ok=1;
